...
export default {
  ...
  methods: {
    ...
    changeStatus(name, active) {
      console.log(this.hero.status,!active, !active ? 60 * +this.timeElapsed.min + +this.timeElapsed.sec : 0)
      this.setStatus({
        id: this.hero.id,
        name,
        active,
        effectStartedAt: !active ? (60 * +this.timeElapsed.min + +this.timeElapsed.sec) : 0
      })
    },
